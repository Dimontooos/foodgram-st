...
from recipes.models import Ingredient, Recipe, IngredientInRecipe, Favorite, ShoppingCart, ShortLink
from .serializers import (
    MinimalUserSerializer, UserSerializer, UserWithRecipesSerializer, SetAvatarSerializer,
    SetAvatarResponseSerializer, IngredientSerializer, RecipeSerializer, RecipeCreateUpdateSerializer,
    RecipeMinifiedSerializer, ShortLinkSerializer, CustomUserCreateSerializer, SetPasswordSerializer,
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(MinimalUserSerializer(user, context={'request': request}).data, status=status.HTTP_201_CREATED)
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Recipe.objects.all().select_related('author').prefetch_related('ingredients')

    def perform_create(self, serializer):
        if not self.request.user.is_authenticated:
            raise ValidationError({"detail": "Требуется аутентификация для создания рецепта."})
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.author != request.user:
            return Response({"detail": "Вы не являетесь автором этого рецепта."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def get_link(self, request, pk=None):
        ...
        short_link, created = ShortLink.objects.get_or_create(
            recipe=recipe, defaults={'short_code': str(uuid.uuid4())[:8]}
        )
        serializer = ShortLinkSerializer(short_link, context={'request': request})
        return Response({'short-link': serializer.data['short_link']}, status=status.HTTP_200_OK)
        ...
        try:
            ...
            if not ingredients:
                ...
                for item in ingredients:
                    name = item.get('ingredient__name') or 'Неизвестный ингредиент'
